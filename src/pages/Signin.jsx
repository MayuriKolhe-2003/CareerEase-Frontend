        ...
        <div className="grid md:grid-cols-2 items-center gap-4 max-md:gap-8 max-w-6xl max-md:max-w-lg w-full p-4 m-4 shadow-md rounded-md">
          ...
          <div className="md:max-w-md w-full px-4 py-4">
            <form onSubmit={handleSubmit}>
              ...
              {/* Password */}
              <InputField label="Password" name="password" type="password" value={formData.password} onChange={handleChange} icon={<FaEye />} />
...
const InputField = ({ label, name, type = "text", value, onChange, icon }) => (
  <div className="mt-2">
    ...
    <div className="relative flex items-center">
      <input name={name} type={type} required className="w-full text-gray-800 text-sm border-b border-gray-300 focus:border-primaryRed pl-2 pr-8 py-3 outline-none" 
        placeholder={`Enter ${label.toLowerCase()}`} value={value} onChange={onChange} />
      {icon && <span className="text-gray-800 w-[18px] h-[18px] absolute right-2">{icon}</span>}
    </div>
  </div>
);
